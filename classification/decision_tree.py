...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a flow-level Decision Tree model on IEC104 CSV files.")
    ...
    parser.add_argument("--max-depth", type=int, default=None, help="Maximum depth for the decision tree.")
    ...
    model = Pipeline(
        steps=[
            ...
            (
                "classifier",
                DecisionTreeClassifier(random_state=args.random_state, max_depth=args.max_depth),
            ),
        ]
    )
    ...
    _save_roc_curve(fpr, tpr, roc_auc, plots_dir / "flow_tree_roc_curve.png")
    pd.DataFrame({"fpr": fpr, "tpr": tpr, "threshold": thresholds}).to_csv(
        outputs_dir / "flow_tree_roc_points.csv", index=False
    )

    _save_confusion_matrix(cm, outputs_dir / "flow_tree_confusion_matrix.csv")
    _save_confusion_matrix_plot(cm, plots_dir / "flow_tree_confusion_matrix.png")
    ...
    feat_imp_df.to_csv(outputs_dir / "flow_tree_feature_importances.csv", index=False)

    # Save a textual tree (limited depth to avoid huge files)
    try:
        tree_text = export_text(classifier, feature_names=feature_cols, max_depth=4)
        (outputs_dir / "flow_tree_tree.txt").write_text(tree_text, encoding="utf-8")
    except Exception:
        pass

    metrics_text = (
        "Flow-level Decision Tree classification (multi-feature)\n"
    ...
    metrics_path = outputs_dir / "flow_tree_metrics.txt"
