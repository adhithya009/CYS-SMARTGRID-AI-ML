...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a flow-level Gradient Boosting model on IEC104 CSV files.")
    ...
    parser.add_argument("--max-depth", type=int, default=3, help="Maximum depth for each boosting tree.")
    parser.add_argument("--subsample", type=float, default=1.0, help="Fraction of samples used for fitting each stage.")
    args = parser.parse_args()

    shared_outputs_dir = PROJECT_ROOT / "outputs"
    outputs_dir = shared_outputs_dir / "gradient_boosting"
    ...
    _save_roc_curve(fpr, tpr, roc_auc, plots_dir / "flow_gradient_boosting_roc_curve.png")
    pd.DataFrame({"fpr": fpr, "tpr": tpr, "threshold": thresholds}).to_csv(
        outputs_dir / "flow_gradient_boosting_roc_points.csv", index=False
    )

    _save_confusion_matrix(cm, outputs_dir / "flow_gradient_boosting_confusion_matrix.csv")
    _save_confusion_matrix_plot(cm, plots_dir / "flow_gradient_boosting_confusion_matrix.png")
    ...
    if hasattr(classifier, "feature_importances_"):
        ...
        feat_imp_df.to_csv(outputs_dir / "flow_gradient_boosting_feature_importances.csv", index=False)

    metrics_text = (
        "Flow-level Gradient Boosting classification (multi-feature)\n"
    ...
    metrics_path = outputs_dir / "flow_gradient_boosting_metrics.txt"
