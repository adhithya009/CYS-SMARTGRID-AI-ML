...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a flow-level KNN model on IEC104 CSV files.")
    ...
    parser.add_argument("--n-neighbors", type=int, default=11, help="Number of neighbors for KNN.")
    parser.add_argument("--weights", type=str, default="distance", choices=["uniform", "distance"], help="Neighbor weights.")
    parser.add_argument("--metric", type=str, default="minkowski", help="Distance metric for KNN.")
    args = parser.parse_args()

    shared_outputs_dir = PROJECT_ROOT / "outputs"
    outputs_dir = shared_outputs_dir / "knn_outputs"
    ...
    _save_roc_curve(fpr, tpr, roc_auc, plots_dir / "flow_knn_roc_curve.png")
    pd.DataFrame({"fpr": fpr, "tpr": tpr, "threshold": thresholds}).to_csv(
        outputs_dir / "flow_knn_roc_points.csv", index=False
    )

    _save_confusion_matrix(cm, outputs_dir / "flow_knn_confusion_matrix.csv")
    _save_confusion_matrix_plot(cm, plots_dir / "flow_knn_confusion_matrix.png")

    metrics_text = (
        "Flow-level classification with KNN (multi-feature)\n"
        f"Sampled rows: {len(data)}\n"
        f"{source_details}\n"
        f"Feature count: {len(feature_cols)}\n"
        f"KNN config: n_neighbors={args.n_neighbors}, weights={args.weights}, metric={args.metric}\n"
    ...
    print(metrics_text)
    (outputs_dir / "flow_knn_metrics.txt").write_text(metrics_text, encoding="utf-8")
