...
def _build_adaboost(
    ...
    base_tree = DecisionTreeClassifier(max_depth=base_max_depth, random_state=random_state)
    try:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train a flow-level AdaBoost model on IEC104 CSV files.")
    ...
    parser.add_argument("--n-estimators", type=int, default=250, help="Number of boosting rounds.")
    parser.add_argument("--learning-rate", type=float, default=0.1, help="Learning rate for AdaBoost.")
    parser.add_argument("--base-max-depth", type=int, default=2, help="Depth of the base decision tree estimator.")
    args = parser.parse_args()

    shared_outputs_dir = PROJECT_ROOT / "outputs"
    outputs_dir = shared_outputs_dir / "adaboost"
        ...
        steps=[
            ...
            (
                ...
                _build_adaboost(
                    ...
                    base_max_depth=args.base_max_depth,
                ),
            ),
        ]
    )
    ...
    _save_roc_curve(fpr, tpr, roc_auc, plots_dir / "flow_adaboost_roc_curve.png")
    pd.DataFrame({"fpr": fpr, "tpr": tpr, "threshold": thresholds}).to_csv(
        outputs_dir / "flow_adaboost_roc_points.csv", index=False
    )

    _save_confusion_matrix(cm, outputs_dir / "flow_adaboost_confusion_matrix.csv")
    _save_confusion_matrix_plot(cm, plots_dir / "flow_adaboost_confusion_matrix.png")
    ...
    if hasattr(classifier, "feature_importances_"):
        ...
        feat_imp_df.to_csv(outputs_dir / "flow_adaboost_feature_importances.csv", index=False)

    if hasattr(classifier, "estimator_weights_"):
        estimator_weights_df = pd.DataFrame(
            {
                "estimator_idx": np.arange(len(classifier.estimator_weights_)),
                "estimator_weight": classifier.estimator_weights_,
                "estimator_error": classifier.estimator_errors_,
            }
        )
        estimator_weights_df.to_csv(outputs_dir / "flow_adaboost_estimator_weights.csv", index=False)

    metrics_text = (
        "Flow-level AdaBoost classification (multi-feature)\n"
        f"Sampled rows: {len(data)}\n"
        f"{source_details}\n"
        f"Feature count: {len(feature_cols)}\n"
        f"n_estimators: {args.n_estimators}\n"
        f"learning_rate: {args.learning_rate}\n"
        f"base_max_depth: {args.base_max_depth}\n"
    ...
    metrics_path = outputs_dir / "flow_adaboost_metrics.txt"
